...
The practice file for this campaign is broken.ts — cursor on the filename, gf.

broken.ts

That's a real type error on the calculateTotal call — vtsls flagged it the moment the buffer loaded, no action needed from you. There's also a TODO and a FIXME comment further down; you'll use those in a minute.

<leader>xx opens the diagnostics panel. Press it now. Trouble opens at the bottom of the screen, listing every diagnostic from every buffer you currently have open in this session — right now that's just broken.ts, so you'll see the one error. It doesn't scan the whole project on disk, only what's actually loaded; open more files with real errors during a normal editing session and they'd stack up here too, grouped by file: errors, then warnings, then hints. Navigate with j/k. Press <CR> on any entry to jump straight to that line in the source file. Trouble stays open so you can keep navigating. New move unlocked.
...
<leader>xt opens the TODOs panel — and this one works completely differently from <leader>xx. Instead of reading already-loaded buffers, it shells out to ripgrep and actually scans every file in the project on disk for comment markers (TODO, FIXME, HACK, NOTE, etc.), whether you've opened them this session or not. Try it now — it'll find both comments in broken.ts. <leader>xT narrows it further to only TODO/FIX/FIXME, filtering out the informational notes. Good for a pre-commit sweep to make sure you haven't left debug stubs behind. New move unlocked.
